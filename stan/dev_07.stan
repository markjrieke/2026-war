...
data {
    ...
    real beta_v_mu;                 // Non-candidate parameter initial state prior mean
    real beta_v_sigma;              // Non-candidate parameter initial state prior scale
...
parameters {
    ...
    matrix[F,E] beta_v;             // Non-candidate parameter variables per cycle
...
model {
    ...
    target += normal_lpdf(to_vector(beta_v) | beta_v_mu, beta_v_sigma);
