...
data {
    ...
    int<lower=1> E;                         // Number of election cycles
    int<lower=1> C;                         // Number of candidates
    int<lower=1> F;                         // Number of non-candidate variables
    ...
    // Mapping columns
    array[N,2] int<lower=1, upper=C> cid;   // Map candidates to races
    array[N] int<lower=1, upper=E> eid;     // Map election cyle to race

    // Model config flags
    int<lower=1> FEC;                       // Column position of fec information in design matrix
    array[N] int<lower=0, upper=1> fec;     // Whether or not fec information is available
...
transformed data {
    ...
    // For fec, uses the mean/sd conditional on having fec data
    matrix[N,F] Xc;
    vector[F] X_mean;
    vector[F] X_sd;
    for (f in 1:F) {
    ...
    // Adjust design matrix based model config flags
    for (n in 1:N) {
        if (fec[n] == 1) {
            Xc[n,FEC] = 0;
        }
